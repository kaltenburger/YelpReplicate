...
if CV_model:
    file_output = open('.results/kang_results_regression_februaryCVuniversalembedding_SEED_ndim_'+str(ndim)+'_window_'+str(window)+'_min_count_'+str(min_count)+ model_type+'_search_'+search_type+'.csv', 'wt')
...
feature_set = np.array([
                        np.array(embedding_df.columns[embedding_df.columns!='inspection_id'])
                        ])
...
for subset in feature_set:
    ...
    for train, test in kf.split(dta_analysis):
        ...
        m = m+1
        if CV_model:
            writer.writerow( (m, subset, train,model_full.best_params_.values(), #model_full.cv_results_,
                              mean_absolute_error(y_test, y_predict),
                              mean_squared_error(y_test, y_predict)))
        else:
            writer.writerow( (m, subset, train,'0',
                          mean_absolute_error(y_test, y_predict),
                          mean_squared_error(y_test, y_predict)))
    print ''
