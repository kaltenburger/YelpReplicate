...
# save output
file_output = open('./results/kang_results_regression_CV_model_SEED_'+str(CV_model)+ '_all_and_embed_'+str(ndim)+'_window_'+str(window)+'_min_count_'+str(min_count)+ model_type+'_search_'+search_type+'.csv', 'wt')
...
## Temporary for manually checking feature importance
importances = model_full.best_estimator_.feature_importances_
indices = np.argsort(importances)[::-1]
print("Feature ranking:")
for f in range(10):#range(len(x_train.columns)):
    print((f + 1, np.array(x_train.columns)[indices[f]], importances[indices[f]]))
