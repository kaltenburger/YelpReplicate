...
if CV_model:
    if model_type=='RF':
        ...
        if search_type == 'grid':
            model_full = GridSearchCV(estimator = clf, random_state = 123,
    ...
    if model_type =='SVR':
        ...
        param_grid = {'C': Cs}
        model_full = GridSearchCV(estimator = SVR(), param_grid = param_grid, cv=kf, random_state = 123,
...
# save output
file_output = open('./results/kang_results_regression_CV_model_SEED_'+str(CV_model)+ '_Yelp_cuisine_and_zip_'+str(ndim)+'_window_'+str(window)+'_min_count_'+str(min_count)+ model_type+'_search_'+search_type+'.csv', 'wt')
